 ...
 * - Collection type: /api/why-sections?populate=*
 */
export async function getWhySection() {
  const json = await fetchFromCMS("/api/why-sections?populate=*");
  ...
  if (!item) {
    return {
      ...
      sectionSubtitle:
        "Une expertise unique pour un événement qui vous ressemble",
      items: [],
    };
  }
  ...
  const {
    sectionTitle = "Pourquoi nous choisir ?",
    sectionSubtitle = "Une expertise unique pour un événement qui vous ressemble",
    why_items = [],
  } = item;

  const items = Array.isArray(why_items)
    ? why_items
        .map((i) => ({
          id: i.id,
          order: i.order ?? 0,
...
export async function getInteriorConfig() {
  ...
  if (!item) {
    return {
      ...
      sectionSubtitle: "Des services sur-mesure pour sublimer vos intérieurs",
      sliderImages: [],
    };
  }
...
export async function getEventConfig() {
  ...
  if (!item) {
    return {
      ...
      sectionSubtitle: "Des décors sur-mesure pour tous vos événements",
      sliderImages: [],
    };
  }
...
export async function getDevisPage() {
  const json = await fetchFromCMS("/api/devis-pages?populate=teamImage");
  ...
  return {
    ...
    teamImage: buildMediaUrl(item.teamImage),
  };
}

// 🔹 Récupérer les infos du calendrier de devis
export async function getDevisCalendar() {
  // adapte le path si Strapi a créé un autre nom (ex: /api/devis-calendars)
  const json = await fetchFromCMS("/api/devis-calendars");
  ...
  if (!item) {
    return {
      ...
      subtitle: "Prochaines dates pour vos prestations",
      busyDates: [],
      vacationDates: [],
    };
  }
  ...
  return {
    title: item.title || "Disponibilités",
    subtitle: item.subtitle || "Prochaines dates pour vos prestations",
